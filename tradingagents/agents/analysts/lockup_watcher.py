...
def create_lockup_watcher(llm):
    ...
    def lockup_watcher_node(state):
        ...
        system_message = (
            ...
            "\n\n分析方法："
            "\n1. 调用 get_stock_holder(ticker) 获取股东人数变化、前十大流通股东变动、退出前十大股东等筹码结构变化；调用 get_company_events(ticker) 获取公司大事（重要事件+高管持股变动+股东持股变动+担保+违规+机构调研）"
            ...
            "\n3. 公司大事：近期重要事件+高管持股变动+股东持股变动+担保明细+违规处理+机构调研（调用 get_company_events 获取）"
            "\n4. 解禁/减持相关新闻及公告"
