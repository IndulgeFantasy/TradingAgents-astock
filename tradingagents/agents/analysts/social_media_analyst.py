...
from tradingagents.agents.utils.agent_utils import build_instrument_context, get_fund_flow, get_industry_hotmap, get_language_instruction, get_market_context, get_news, retry_report_generation
...
def create_social_media_analyst(llm):
    def social_media_analyst_node(state):
        ...
        system_message = (
            ...
            "\n- `get_industry_hotmap(level='bk2', top_n=20, ticker='目标股票6位代码')`：获取大盘星图行业热力（全市场个股按行业聚合：涨跌家数/流通市值加权涨跌幅/主力净占比/换手率/领涨领跌股，返回值会标注目标个股所属行业及其排名），用于判断市场情绪广度与板块轮动热度——普涨普跌反映一致性情绪（一致性过强可能是反转信号），行业主力净占比反映资金接力意愿。"
            "\n\n⚠️ 分析纪律 —— 情绪判断必须落在数据上，不能只凭新闻语气推断："
            "\n- **先看资金，再看新闻**。新闻只能解释情绪从哪来，不能单独作为情绪结论的依据。"
            "\n- **背离必须单列指出**：判断「资金面 vs 消息面」是否一致——消息面正面但主力资金持续净流出（或相反），是本报告最有价值的信号，务必单列说明方向并给出解读。"
            "\n- **区分「热度」与「情绪方向」**：上榜、放量只说明关注度高，方向要由资金流向和涨跌结构判断。恐慌抛售同样是高热度。"
            "\n- 数据取不到时如实标注 [数据缺失: xxx]，**不要用新闻语气去补一个编造的数字**。"
