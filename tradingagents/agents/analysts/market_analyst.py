...
📈 行业轮动：调用 get_industry_hotmap(level='bk2', top_n=20, ticker='目标股票6位代码') 获取大盘星图行业热力（全市场个股按行业聚合：涨跌家数/流通市值加权涨跌幅/主力净占比/换手率/领涨领跌股），返回值会标注目标个股所属行业及其排名，确认目标个股所属行业的板块强弱与主力资金集中度，判断个股技术信号是否与行业趋势共振。
...
2. **必须**调用 get_stock_kline_full(code, days) 获取 K 线数据（含换手率/涨跌幅/ST标记），**days 参数一律传 {lookback}**（这是用户指定的分析回溯区间，覆盖工具默认值120）
3. 调用 get_indicators 获取选定指标（参数名使用上述英文标识符，否则调用会失败），**look_back_days 参数一律传 {lookback}**
